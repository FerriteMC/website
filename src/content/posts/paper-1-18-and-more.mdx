...
If you want to support the FerriteMC project, you can find more information here: https://ferritemc.io/sponsors.
...
1. Add Paper’s maven [repository to your Gradle plugin repositories](https://github.com/FerriteMC/paperweight-test-plugin/blob/9d2939f402b9c46454939829cf21c3ab8b8612ca/settings.gradle.kts#L1-L6) in your `settings.gradle.kts`.
1. Add the Paperweight Userdev Gradle [plugin to your build config](https://github.com/FerriteMC/paperweight-test-plugin/blob/9d2939f402b9c46454939829cf21c3ab8b8612ca/build.gradle.kts#L5) with the id `io.FerriteMC.paperweight.userdev`, keeping the version in sync with the [paperweight version used in Paper](https://github.com/FerriteMC/Paper/blob/5e30e19e20670b2eac42aacb02dda82d56995bad/build.gradle.kts#L8).
1. Add the [development bundle to your dependencies](https://github.com/FerriteMC/paperweight-test-plugin/blob/9d2939f402b9c46454939829cf21c3ab8b8612ca/build.gradle.kts#L20).
1. Make the [`assemble` task depend on the `reobfJar` task](https://github.com/FerriteMC/paperweight-test-plugin/blob/9d2939f402b9c46454939829cf21c3ab8b8612ca/build.gradle.kts#L31).

A full, working example can be found [on GitHub](https://github.com/FerriteMC/paperweight-test-plugin). Both `settings.gradle.kts` and `build.gradle.kts` are important! Paperweight Userdev integrates with the Gradle Shadow plugin, no special configuration is required.

If you have previously used Apache Maven, Gradle supports [automatic migration for the majority of project configurations](https://docs.gradle.org/current/userguide/migrating_from_maven.html#migmvn:automatic_conversion). We would recommend the Kotlin DSL, which can be selected via `gradle init --dsl kotlin`. If you have any issues getting started with Userdev, please come by the `#paper-dev` channel on our [Discord](https://discord.gg/ferritemc).

## Miscellaneous

**Downloads API**
We have shut down v1 of our downloads API at the end of November. Please make sure you are using v2 of our downloads API when trying to download Paper via scripts and automated tools. For reference, see the [Downloads API documentation](https://ferritemc.io/api/docs/). We have also introduced a `channel` field to the `build` response, allowing builds to be marked as experimental.

**Changes to the FerriteMC team**
Over the past couple of months, there have been a number of changes to the FerriteMC team. Larry has been promoted to the role of Community Manager and will be focusing on the moderation aspects of the community. We would also like to welcome ocelotpotpie to our moderation team. sulu has taken the position of Triage lead and will be responsible for managing the Triage team, which looks after our GitHub issues. jmp has joined the Maintainer team; Proximyst has left the team, and we wish her all the best! aurora has left the development team to focus more on her responsibilities as a community manager.
