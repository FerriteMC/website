...
Once these changes are present in stable Paper builds, you can expect a much better experience when using the often dreaded internals by using our [userdev](https://docs.ferritemc.io/paper/dev/userdev) Gradle plugin. Most notably, small Minecraft updates will no longer unconditionally break your plugins.
...
If you are running a somewhat recent server version with up-to-date plugins (!), you should also test whether your plugins are able to run on the [test server jar](https://github.com/FerriteMC/testing/releases/tag/no-relocation) also linked above. **Please make sure not to use the server jar on your main servers, but to copy your plugin setup to a separate test server.**
